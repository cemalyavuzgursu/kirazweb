...
import { loginCustomer } from "@/server/actions/customer-account";
...
export default function CustomerLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      await loginCustomer(new FormData(e.currentTarget));
    } catch (err) {
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        <h1 className="font-display text-3xl mb-2" style={{ color: "var(--kt-heading)" }}>Hesabıma Giriş</h1>
        <p className="text-sm mb-8" style={{ color: "var(--kt-muted)" }}>
          Hesabın yok mu?{" "}
          <Link href="/hesabim/kayit" className="hover:underline" style={{ color: "var(--kt-primary)" }}>Kayıt Ol</Link>
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 rounded-lg p-6" style={{ border: "1px solid var(--kt-border)" }}>
          ...
          <div>
            <label className="block text-sm font-medium mb-1" htmlFor="password" style={{ color: "var(--kt-heading)" }}>Şifre</label>
          ...
          <p className="text-sm text-center" style={{ color: "var(--kt-muted)" }}>
            <Link href="/hesabim/sifremi-unuttum" className="hover:underline" style={{ color: "var(--kt-primary)" }}>Şifremi Unuttum</Link>
          </p>
        </form>
      </div>
    </div>
